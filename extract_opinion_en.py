...
class OpinionExtractor:
    ...
    def _get_after_implicit(self,actual_index: [], priori_value: float, threshold: float):
        '''
        Get index scope after explicit/implicit judgment.
        :param actual_index: the actual index of opinion
        :param priori_value: the prior sentiment value of opinion
        :param threshold:
        :return:
        '''
        if priori_value < threshold:
            after_implicit = [-1, -1]
        else:
            after_implicit = actual_index
        return after_implicit
    ...
    def extract_clause_opinion(self, std_p: float = 0.7, threshold: float=0.2):
        '''
        Select one opinion for each clause
        :param std_p: Coefficient of standard deviation, default value is 0.7
        :param threshold: If the representative word is regarded as an opinion (is_potential_aspect=false), it determines whether it is explicit ot implicit.
        ...
        for data in self.data_list:
            ...
            for clause_words_info in clause_words_info_list:
                ...
                opinion = {'actual_index': [clause_opinion[4][0], clause_opinion[4][-1] + 1],
                           'after_implicit': self._get_after_implicit(
                               actual_index=[clause_opinion[4][0],clause_opinion[4][-1] + 1],
                               priori_value = self._get_priori_value_with_senti(word=clause_opinion[0], pos=clause_opinion[1], pola=senti_pola),
                               threshold = threshold),
                           'is_potential_aspect': self._is_potential_aspect(priori_value=self._get_priori_value(word=clause_opinion[0]), pos=clause_opinion[1])}
            ...
            if len(one_text_opinion_list)==0:
                if max_clause_opinion is not None:
                    opinion = {'actual_index': [max_clause_opinion[4][0], max_clause_opinion[4][-1] + 1],
                               'after_implicit': self._get_after_implicit(
                                   actual_index=[max_clause_opinion[4][0], max_clause_opinion[4][-1] + 1],
                                   priori_value=self._get_priori_value_with_senti(word=max_clause_opinion[0],pos=max_clause_opinion[1],pola=senti_pola),threshold=threshold),
                               'is_potential_aspect': self._is_potential_aspect(priori_value=self._get_priori_value(word=max_clause_opinion[0]), pos=max_clause_opinion[1])}
    ...
    def extract_top_k_opinion(self, threshold: float, k: int):
        """
            ...
            if len(words_info) == 0:
                ...
                for x in temp[0:k]:
                    opinion = {'actual_index': [x[4][0], x[4][-1] + 1],
                               'after_implicit': self._get_after_implicit(
                                                        actual_index=[x[4][0], x[4][-1] + 1],
                                                        priori_value=self._get_priori_value_with_senti(word=x[0],pos=x[1],pola=senti_pola),
                                                        threshold=threshold),
    ...
    def _load_data(self):
        ...
        with StanfordCoreNLP(STANFORD_CORE_NLP_PATH) as nlp:
            for text, shaps, sentiment in zip(text_list, shap_list, sentiment_list):
                ...
                iter_pos, iter_shap, iter_index = nlp.pos_tag(sentence), map(lambda x: (x[0].strip(), x[1]), shaps), map(lambda x: (x[1], x[0]), enumerate(text.split(' ')))
...
def analysis_shap_2(target_name: str,model: Model, text_list: list, sentiment_list: list):
    # Load parsed files：
    tmp_fname = f'{BASE_DIR}/data/pred_opinion/{target_name}_test_shaps_tmp.txt'
    output_fname = f'{BASE_DIR}/data/pred_opinion/{target_name}_test_shaps.pkl'
    parsed_text_list = []
...
def main():
    ...
    # # Load the best model.
    best_model = load_model(f'{target_name}_model', (2, 2, 3), (300, 200, 200), (2, 1, 1), 150)
    # Read the predicted sentiment list.
    ...
    text_list = list(text_sentiment_data['text'])
    sentiment_list = list(text_sentiment_data['sentiment'])
    analysis_shap_2(target_name,best_model, text_list, sentiment_list)
    ...
    sentiment_fname = f'{BASE_DIR}/data/pred_senti/{target_name}_test_sentiment.csv'
    shap_file = f'{BASE_DIR}/data/pred_opinion/{target_name}_quad_test_shaps.pkl'
    extractor = OpinionExtractor(sentiment_fname,shap_file)
    text_list = extractor.get_texts()
    opinion_list = extractor.extract_clause_opinion(std_p=0.7, threshold=0.2)
    with open(opinion_fpath, 'w') as file:
