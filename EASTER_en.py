...
def analysis_sentiment(target_name):
    ...
    model = build_model((2, 2, 3), (300, 200, 200), (2, 1, 1), 150, include_text_cnn)
    train_model(f'{target_name}', f'{target_name}_model', model, 30)
    ...
    with open(f'{BASE_DIR}/data/acos/{target_name}_quad_test.tsv', 'r', encoding='utf-8') as f:
    ...
    sentiment_list = list(
        map(lambda x: int(x) - 1, np.where(sentiment_list == np.max(sentiment_list, axis=1).reshape(-1, 1))[1]))
    ...
    sentiment_df.to_csv(sentiment_fname, index=False)
...
if __name__ == "__main__":
    ...
    """
    target_name = "laptop"
    analysis_sentiment(target_name);
