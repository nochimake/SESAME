...
class Evaluater:
    ...
    def _quota(self, pred_list: list, gold_list: list,aos_map, data_map, is_true):
        TP = FP = GTP = GFN = 0
        for pred_aos, real_aos in zip(map(lambda x: set(map(aos_map, x['aos_list'])), pred_list),
                                      map(lambda x: set(map(data_map, x['aos_list'])), gold_list)):
    ...
    def _parse_line(self,line):
        ...
        if self.EVALUATE_TYPE == self.EVALUATE_ACOS:
            ...
            line_res['aos_list'] = parsed_entries
        elif self.EVALUATE_TYPE == self.EVALUATE_AS:
...
def evaluate():
    target_name = 'laptop'
    human_label_fname = f'{BASE_DIR}/data/acos/{target_name}_quad_test.tsv'
    pred_aos_fname = f'{BASE_DIR}/data/pred_aspect/{target_name}_aos.txt'

    # target_name = 'camera'
    # human_label_fname = f'{BASE_DIR}/data/acos/{target_name}_as_test.tsv'
    # pred_aos_fname = f'{BASE_DIR}/data/pred_aspect/{target_name}_aos.txt'
    ...
    print("Index Range:")
    # print("AOS Strict:")
    # result = extractor.quota_aos_index(aos_list, 'strict')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # print("AOS Loose:")
    # result = extractor.quota_aos_index(aos_list, 'loose')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # #
    print("AS Strict:")
    result = extractor.quota_as_index(aos_list, 'strict')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    print("AS Loose:")
    result = extractor.quota_as_index(aos_list, 'loose')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    #
    print("A Strict:")
    result = extractor.quota_a_index(aos_list, 'strict')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    print("A Loose:")
    result = extractor.quota_a_index(aos_list, 'loose')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    #
    # print("O Strict:")
    # result = extractor.quota_o_index(aos_list, 'strict')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # print("O Loose:")
    # result = extractor.quota_o_index(aos_list, 'loose')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    ...
    # print("AOS Strict:")
    # result = extractor.quota_aos_text(aos_list, is_need_textify, 'strict')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # print("AOS Loose:")
    # result = extractor.quota_aos_text(aos_list, is_need_textify, 'loose')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # #
    print("AS Strict:")
    ...
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    print("AS Loose:")
    result = extractor.quota_as_text(aos_list, is_need_textify, 'loose')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    #
    print("A Strict:")
    result = extractor.quota_a_text(aos_list, is_need_textify, 'strict')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    print("A Loose:")
    result = extractor.quota_a_text(aos_list, is_need_textify, 'loose')
    print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # #
    # print("O Strict:")
    # result = extractor.quota_o_text(aos_list, 'strict')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
    # print("O Loose:")
    # result = extractor.quota_o_text(aos_list, 'loose')
    # print(f'{result[0]}\t{result[1]}\t{result[2]}')
